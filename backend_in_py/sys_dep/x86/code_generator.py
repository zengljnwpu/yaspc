...
class CodeGenerator ():
    ...
    def __does_require_register_operand (self, op):
        if op == Op.S_DIV or op == Op.U_DIV or op == Op.S_MOD or op == Op.U_MOD or op == Op.BIT_LSHIFT or op == Op.BIT_RSHIFT or op == Op.AIRTH_RSHIFT:
            return True
        else:
            return False
    def __compile_binary_op (self, op, left, right):
        if op == Op.ADD:
            ...
            if op == Op.EQ: self.__as.sete (self.__al())
            elif op == Op.NEQ: self.__as.setne (self.__al())
            elif op == Op.S_GT: self.__as.setg (self.__al())
            elif op == Op.S_GTEQ: self.__as.setge (self.__al())
            elif op == Op.S_LT : self.__as.setl (self.__al())
            elif op == Op.S_LTEQ: self.__as.setle (self.__al())
            elif op == Op.U_GT: self.__as.seta (self.__al())
            elif op == Op.U_GTEQ: self.__as.ae (self.__al())
            elif op == Op.U_LT: self.__as.setb (self.__al())
            elif op == Op.U_LTEQ: self.__as.setbe (self.__al())
            else:
                raise ArithmeticError
