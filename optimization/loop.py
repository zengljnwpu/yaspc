...
class LoopOptimizer(function_optimizer.FunctionOptimizer):
    """LoopOptimizer class
    """
    @classmethod
    def find_dominator(cls, block_list):
    ...
    @classmethod
    def __insert(cls, n, loop_set, stack):
        if n not in loop_set:
            loop_set.add(n)
            stack.append(n)

    @classmethod
    def find_loop(cls, d, n, block_list):
        """给定回边 n->d，确定此循环的全部节点
        ...
        loop_set.add(d)
        cls.__insert(n, loop_set, stack)
        while len(stack) > 0:
            m = stack.pop()
            block = block_list[m]
            for pre_block in block.preBasicBlock:
                cls.__insert(pre_block.blockNum, loop_set, stack)
        return loop_set

    def do_loop_optimization(self):
        """the main function of Loop optimization
        """
        block_list = self.data_unit.get_block_list()

        # find dominator
        D = self.find_dominator(block_list)
            ...
            for d in dom_set:
                for succ, description in block_list[n].succBasicBlock:
                    ...
                    if succ == block_list[d]:
                        ...
                        loop_set = self.find_loop(d, n, block_list)
        ...
        for loop in loop_list:
            ...
            self.analysis_live_variable()

            var_reduce = dict()
            var_table = self.data_unit.get_var_table()
            for key in var_table:
                var_reduce[key] = var_table[key]['var_reduce']
